...
r_loop:
	...
	mov ax,320		;ax=320
	mul r			;ax=320*r
	add py,ax			;py=p+320*r
	
	mov c,0			;初始化c，开始循环
c_loop:
	mov ax,mid		;ax=mid
	mov di,ax			;di=mid
	mov al,asc[di]		;al=asc[mid]
	mov ah,0			;ah初始化
	mov bx,1			;bx=1
	mov cl,7			;cl=7
	mov dx,c			;dx=c
...
next:
	...
	mov cx,c			;cx=c
	add px,cx			;px=py+c
	mov bx,0A000h		;bx=0xA000
	mov es,bx		;es=0xA000
	mov bx,px		;bx=px
	mov byte ptr es:[bx],al	;将地址为es:bx出赋值为对应颜色
...
gets:
	...
	cmp al,13			;判断是否是回车
	je EOF
	mov buf[si],al		;传入buf数组
	add si,1			;数组下标++
	jmp gets			;跳回gets，继续读入
...
pic:
	mov al,buf[si]		;判断是否是结尾
	cmp al,0
	je ending

	mov al,buf[si]		;al=buf[si]
	mov ah,0			;初始化ah
	sal ax,4			;ax=buf[si]<<4
	mov mid,ax		;mid=buf[si]<<4
	call draw_a_char		;调用函数
	add x,8			;x+=8
	cmp x,320		;判定x是否大于等于320
	jae over
	jmp event
